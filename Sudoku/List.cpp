...
List::List(float x_pos, float y_pos, float width, float height, std::string options[], unsigned noOFoptions, float gridSizef, std::string firstOption, unsigned activeButton)
{
	...
	this->selectedOption = new Button(this->settings.soundLevel,
		x_pos, y_pos,
		width, height,
		firstOption, this->font, gridSizef,
...
void List::update(sf::Vector2f& mousePos, const float& dt, sf::RenderWindow& window, sf::Event event)
{
	...
	{
		this->delta += event.mouseWheel.delta;
		std::cout << 1 << std::endl;
	}*/
